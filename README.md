...
| `src/ScienceTimeline.Etl` | .NET 10, Npgsql | Импорт из Wikidata (SPARQL) и Crossref |
| `src/ScienceTimeline.Api` | .NET 10, Dapper | HTTP API: бакеты, карточка, поиск |
| `web` | Vite, TypeScript, Canvas | Лента |
...
API и фронтенд в двух терминалах:

```bash
dotnet run --project src/ScienceTimeline.Api
```
...
| Метод | Ответ |
|---|---|
| `GET /api/meta` | границы времени, справочник областей, типы событий |
| `GET /api/timeline?from&to&buckets&topk&kinds&categories` | счётчики по бакетам и топ-K значимых в каждом |
| `GET /api/events?from&to&limit&offset` | плоский список событий за интервал |
| `GET /api/events/{id}` | карточка: авторы, цепочка дат, ссылки |
| `GET /api/search?q` | полнотекстовый поиск сразу по русскому и английскому |

`from` и `to` — сутки от 1970-01-01. Основной запрос отдаёт и разбивку
по типам для подписи кластера, и сами события — за один проход по индексу
`events (t_mid) include (significance, kind)`.
